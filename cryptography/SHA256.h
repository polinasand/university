...
class SHA256 {
    ...
    const static byte endMessageByte = byte(128);
	const static byte zeroByte = byte(0);
    const unsigned int blockSize = 512;
    const static unsigned int numRounds = 64;
    const unsigned long mod = 4294967296;
    void init();
    byte* preprocess(byte* message, int message_bytes);
    void processBlock(byte* block, unsigned int, unsigned int * w);
public:
    unsigned int* hash(string message);
};
*/
#pragma once
#include <string>
